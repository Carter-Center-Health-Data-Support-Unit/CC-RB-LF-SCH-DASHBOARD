...
let's see if we can somehow double check with a viz
```{r}
...
debugonce(extract_pre_clean_names_adms_batch)
what <- extract_pre_clean_names_adms_batch(df_list =RB_post201905_df_ls,
                                           data_format = "current" ) %>% suppressMessages()



RB_post201905_adm3 %>% 
  filter(adm1_name =="amhara", year %in% c(2020,2021), month %in% c(11,12,1))


RB_pre201905_adm2 %>% 
  filter(year %in% c(2017,2018), month %in% c(9,10,11,12,1,2), adm2_name =="yem_special")


adm2_spillover_identified %>% write.table(., "clipboard", sep="\t",mode="r")



RB_post201905_adm3 %>% 
  # filter(adm3_name %in% adm3_spillovers_identified$adm3_name,
  #        year>=2019) %>% 
  filter(adm1_name =="gambela") %>% 
  ggplot(aes(x= date, y= popn_treated_during_current_month))+
  geom_line(aes(color=adm3_name)) +
  geom_point(aes(color=adm3_name))
