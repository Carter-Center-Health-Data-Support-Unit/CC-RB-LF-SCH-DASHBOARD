...
# these are adm2 level, but after may 2019 everything was 
# admin 3.. therefore the problem shoudl be adm3 after may 2019
year_comparisons |>
  filter(min_cum!=0) |> 
...
adm2_spillover_identified <- year_comparisons |>
  filter(min_cum!=0) |> 
  filter(diff_range<5000) |> 
  rename(start_of_year=date_set) |> 
  distinct(adm1_name,adm2_name,diff_range,start_of_year ) |> 
  ...
  arrange(adm1_name,adm2_name,start_of_year) |> 
  select(-diff_range) %>% 
  filter(start_of_year <=2019)

adm3_spillovers_identified <- year_comparisons_adm3 |>
  filter(min_cum!=0) |> 
  filter(diff_range<5000) |> 
  rename(start_of_year=date_set) |> 
  distinct(adm1_name,adm2_name,diff_range,start_of_year ) |> 
...
here i'm checking and writing out tables for an email
```{r, eval = F}

clip <- pipe("pbcopy", "w")                       
write.table(adm2_spillover_identified, file=clip, sep = '\t', row.names = FALSE)                               
close(clip)
 
eth_master_adm %>% 
  filter(
  str_detect(adm3_en,"qu")
  )
```

How can we rectify these in the data?
```{r}

RB_post201905_adm3$popn_treated_during_current_month_fix
RB_post201905_adm3$popn_treated_during_current_month
RB_pre_post_compiled$popn_treated_during_current_month
RB_pre_post_compiled$popn_treated_during_current_month_fix
...
debugonce(id_spillovers)
id_spillovers(RB_post201905_adm3_dedup,x = "popn_treated_during_current_month",grp_vars = c("adm1_name","adm2_name","adm3_name","year"))

fix_spillovers(RB_post201905_adm3_dedup,x = "popn_treated_during_current_month",grp_vars = c("adm1_name","adm2_name","adm3_name","year")) %>% glimpse()


debugonce(id_spillovers)
id_spillovers(RB_pre201905_to_adm2,
              x = "popn_treated_during_current_month",grp_vars = c("adm1_name","adm2_name","year"))
fix_spillovers(RB_pre201905_to_adm2,
              x = "popn_treated_during_current_month",grp_vars = c("adm1_name","adm2_name","year"))

RB_pre201905_adm2$popn_treated_during_current_month_fix
...
heat_map_gg(.dat = RB_pre_post_compiled,x = "popn_treated_during_current_month",grp_vars = c("year","adm1_name","adm2_name"),date_col = "date")
heat_map_gg(.dat = RB_pre_post_compiled,x = "popn_treated_during_current_month",grp_vars = c("year","adm1_name","adm2_name"),date_col = "date")
