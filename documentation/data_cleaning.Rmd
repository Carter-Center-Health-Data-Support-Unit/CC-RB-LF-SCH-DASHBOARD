...
pre201905_clean_phase_1b <- pre_201905_rbrx_clean1 |> 
  purrr::map2_dfr(.y= names(pre_201905_rbrx_clean1), ~{
    print(.y)
              df_names_harmonized <-rename_cols_with_lookup(df = .x,lookup = rename_lookup,lookup_fixed = F)
              df_names_harmonized |> 
                dplyr::mutate(
                  file_name= .y,
...
```{r}
pre201905_clean_phase_1b$adm2_name |> tabyl()
pre_post_compiled |> 
  glimpse()
pre201905_clean_phase_1b |> 
  filter(
    adm2_name=="7_kebeles_in_surma_district_of_bench_maji_zone_were_not_included_in_the_cdti_due_to_nomadic_nature_of_the_people_and_out_of_those_treated_3_villages_were_not_treated_due_to_security_problem"
  )
