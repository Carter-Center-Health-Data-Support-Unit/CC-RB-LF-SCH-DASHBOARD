...
## Intro
The `{targets}` pipeline has been set up to clean `RB rx` data and it should be quite similar for `LF rx` data. Therefore, there is no need to re-document all the cleaning steps in this doc. Rather, it makes sense to run the `{targets}` pipeline with th `LF rx` data and use this document to check the outputs, if any additional functions are needed they can be produced and explained out of this document


Name harmonization: all admin1 good
```{r}
check_admin1(df = LFrx_post201905_df_compiled,df_master = eth_master_adm)
```


On first run when we used name harmonization created for LB rx data, there were some new lookup pairs needed. Put together this patch and raised a GH issue. Could consider integrating into larger `case_when()` or keeping as function
```{r}
...
#         dplyr::case_when(
#           # snnp-gofa-mekeloza  ~ ?
#           "snnp-gofa-melogada"~ "melo_gada"
#           "snnp-south_omo-baka_dawula_ari"~"boko_dawula",
#           "snnp-south_omo-bena_tse_june " ~"bena_tsemay",
#           "oromia-ilu_aba_bora-alge_sache "~"alge_sachi",
#           "oromia-ilu_aba_bora-boilo_nopa"~ "bilo_nopha",
#           "amhara-south_gondar-muja" ~ "sede_muja"
# 
#         )
#       
#     )
...
```{r}
check_admin2(df = LFrx_post201905_df_compiled,
             df_master = eth_master_adm) |> 
  print(n=89)

```

mostly dam_workers and refugee classificatoins which are not in official eth master admin... let's focus on others first

```{r}

#half worked
check_admin3(df = LFrx_post201905_df_compiled,
             df_master = eth_master_adm) 


RB_post201905_adm3 |> 
  filter(str_detect(adm_1_2_3,"dam_work"))

"benishangul_gumz-dam_workers_metekel-dam_workers"~"dam_workers"
"dam_workers_metekel"~"metekel"


# snnp-gofa-mekeloza  ~ snnp-gofa-melekoza
# "snnp-gofa-melogada"~ "snnp-gofa-melo_gada"
# "snnp-south_omo-baka_dawula_ari"~"snnp-south_omo-boko_dawula",
# "snnp-south_omo-bena_tse_june " ~"snnp-south_omo-bena_tsemay",
# "oromia-ilu_aba_bora-alge_sache "~"oromia-ilu_aba_bora-alge_sachi",
# "oromia-ilu_aba_bora-boilo_nopa"~" oromia-ilu_aba_bora-bilo_nopha",
# "amhara-south_gondar-muja" ~ "amhara-south_gondar-sede_muja"
# "snnp-south_omo-selamego "~"snnp-south_omo-salamago"

clean_adm2()
eth_master_adm |> 
  filter(str_detect(adm1_en,"amhara")) |> count(adm2_en)
eth_master_adm |> 
  filter(str_detect(adm2_en,"south_omo")) |> 
  count(adm_1_2_3)

RB_post201905_adm3 |> 
  filter(str_detect(adm3_name,"sout_omo")) |> 
  count(adm1_name)


LFrx_post201905_df_compiled |> 
  filter(str_detect(adm2_name,"metekel")) |> 
  count(adm1_name,adm2_name, adm3_name)


LFrx_post201905_df_compiled |> 
  clean_adm2_lfrx_patch() |> 
  filter(str_detect(adm2_name,"metekel")) |> 
  count(adm1_name,adm2_name, adm3_name)
